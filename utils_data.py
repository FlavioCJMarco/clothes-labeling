...
def visualize_retrieval(imgs, topN, info=None, ok=None, title='', query=None, sorting_element=None, sorting=None):
    ...
    for i in range(min(topN, len(imgs))):
        ...
        if sorting is not None:
            if sorting == "color":  # sorting_element = sorted_probs
                plt.text(rows + 15, columns - 8, "{:.2f}%".format(sorting_element[i]*100), fontsize=8)
            elif sorting == "shape":  # sorting_element = labels_votes
                plt.text(rows + 15, columns - 8, "{} votos".format(sorting_element[i]), fontsize=8)
        plt.axis('off')
        if info is not None:
